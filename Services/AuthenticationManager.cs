...
namespace Services
{
    public class AuthenticationManager : IAuthenticationService
    {
        ...
        {
            var signCredentials = GetSiginCredentials();
    ...
    .CreateAsync(user, userForRegistrationDto.Password);
        ...
        public async Task<IdentityResult> UnregisterUser(UserForAuthenticationDto deleteUserDto)
        {
            var isSucces = await _userManager.FindByNameAsync(deleteUserDto.UserName);
            var result = (_user != null);
            if (!result)
            {
                _logger.LogWarning($"{nameof(ValidateUser)} : User is not found");
                return IdentityResult.Failed(new IdentityError { Description = "User is not found" });
            }
            else
            {
                var deletionResult = await _userManager.DeleteAsync(isSucces);
                if (!deletionResult.Succeeded)
                {
                    _logger.LogError("User deletion failed");
                    return IdentityResult.Failed(new IdentityError { Description = "User deletion failed" });
                }
